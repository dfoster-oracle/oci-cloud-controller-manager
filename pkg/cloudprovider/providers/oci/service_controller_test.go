...
func newEndpointSlice(name string, serviceName string) *discovery.EndpointSlice {
...
func newServiceController(options ...func(*ServiceController)) (*ServiceController, *fakecloud.Cloud, *fake.Clientset) {
	...
	controller := &ServiceController{
		...
		endpointSliceUpdatesBatchPeriod: 0 * time.Second,
	...
	controller.endpointSliceListerSynced = alwaysReady
...
func TestSyncLoadBalancerIfNeeded(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			defer cancel()
			controller, cloud, client := newServiceController()
...
func TestUpdateNodesInExternalLoadBalancer(t *testing.T) {
	...
	for _, item := range table {
		t.Run(item.desc, func(t *testing.T) {
			ctx, cancel := context.WithCancel(context.Background())
			defer cancel()
			controller, cloud, _ := newServiceController()
...
func TestProcessServiceCreateOrUpdate(t *testing.T) {
	controller, _, client := newServiceController()
...
func TestSyncService(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			updateFn: func() {
				controller, _, _ = newServiceController()
			},
...
func TestEndpointsChanged(t *testing.T) {
	testCases := []struct{
	...
	}{
		{
			...
			curEndpoints: []discovery.Endpoint{
				{
					Addresses: []string{"10.0.10.1"},
				},
				{
					Addresses: []string{"10.0.10.2"},
				},
			},
			oldEndpoints: []discovery.Endpoint{
				{
				...
				{
					Addresses: []string{"10.0.10.3"},
				},
			},
			changed: true,
		},
		{
			desc: "same length but different addresses",
			curEndpoints: []discovery.Endpoint{
				{
					Addresses: []string{"10.0.10.1"},
				},
		...
		{
			desc: "same addresses but different order",
			curEndpoints: []discovery.Endpoint{
				{
					Addresses: []string{"10.0.10.1"},
				},
		...
		{
			desc: "endpoints unchanged with empty addresses",
			curEndpoints: []discovery.Endpoint{
				{
					Addresses: []string{},
				},
	...
	for _, testCase := range testCases {
		t.Run(testCase.desc, func(t *testing.T) {
			if testCase.changed != endpointsChanged(testCase.curEndpoints, testCase.oldEndpoints) {
				t.Errorf("excpeted %t, but got %t", testCase.changed, !testCase.changed)
			}
		})
	}
}
