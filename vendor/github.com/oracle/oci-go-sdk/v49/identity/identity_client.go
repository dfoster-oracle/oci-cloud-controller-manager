...
// BulkImportStandardTags OCI will release Tag Namespaces that our customers can import.
// These Tag Namespaces will provide Tags for our customers and Partners to provide consistency and enable data reporting.
func (client IdentityClient) BulkImportStandardTags(ctx context.Context, request BulkImportStandardTagsRequest) (response BulkImportStandardTagsResponse, err error) {
	var ociResponse common.OCIResponse
	policy := common.NoRetryPolicy()
	if client.RetryPolicy() != nil {
		policy = *client.RetryPolicy()
	...
	ociResponse, err = common.Retry(ctx, request, client.bulkImportStandardTags, policy)
	if err != nil {
		if ociResponse != nil {
			if httpResponse := ociResponse.HTTPResponse(); httpResponse != nil {
				opcRequestId := httpResponse.Header.Get("opc-request-id")
				response = BulkImportStandardTagsResponse{RawResponse: httpResponse, OpcRequestId: &opcRequestId}
			} else {
	...
	if convertedResponse, ok := ociResponse.(BulkImportStandardTagsResponse); ok {
		common.EcContext.UpdateEndOfWindow(time.Duration(240 * time.Second))
		response = convertedResponse
	} else {
		err = fmt.Errorf("failed to convert OCIResponse into BulkImportStandardTagsResponse")
	}
	return
}

// bulkImportStandardTags implements the OCIOperation interface (enables retrying operations)
func (client IdentityClient) bulkImportStandardTags(ctx context.Context, request common.OCIRequest, binaryReqBody *common.OCIReadSeekCloser, extraHeaders map[string]string) (common.OCIResponse, error) {

	httpRequest, err := request.HTTPRequest(http.MethodPost, "/tags/actions/bulkImportStandardTags", binaryReqBody, extraHeaders)
	if err != nil {
		return nil, err
	}

	var response BulkImportStandardTagsResponse
	var httpResponse *http.Response
	httpResponse, err = client.Call(ctx, &httpRequest)
	defer common.CloseBodyIfValid(httpResponse)
	response.RawResponse = httpResponse
