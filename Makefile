...
build-local: build-dirs
	@docker run --rm \
		   --privileged \
			 ...
			 -e GOPATH=/go/ \
			odo-docker-signed-local.artifactory.oci.oraclecorp.com/odx-oke/oke/k8-manager-base:go1.19.5-1.0.16 /bin/bash -c \
			'for component in ${COMPONENT}; do \
...
test-local: build-dirs
	@docker run --rm \
		   --privileged \
			 ...
			 -e COMPONENT="$(COMPONENT)" \
			 -e GOPATH=/go/ \
			odo-docker-signed-local.artifactory.oci.oraclecorp.com/odx-oke/oke/k8-manager-base:go1.19.5-1.0.16 \
			make coverage image
